...
These aliases are not just for CLI convenience. If you want your AI assistant to switch between multiple vaults autonomously over the MCP protocol without restarting the server, you can pass these aliases to `obx serve`:

```bash
obx serve --allow-vault-switching --allowed-vaults my-notes,work
```
