---
title: FAQ
description: Frequently asked questions about obsidian-go-mcp.
---
...
### What is obsidian-go-mcp?

obsidian-go-mcp is an MCP (Model Context Protocol) server that lets AI assistants interact with your Obsidian vault. It provides 70 tools for reading, writing, searching, and organizing notes.

### Do I need Obsidian installed?

No. obsidian-go-mcp works directly with Markdown files in your vault folder. Obsidian doesn't need to be installed or running.
...
obsidian-go-mcp itself is completely local. However, when you use it with an AI assistant like Claude, the note content you discuss is sent to that AI provider. See [Security](/obsidian-go-mcp/advanced/security) for details.

## Installation

### How do I install on Windows?

Download the Windows binary from [GitHub Releases](https://github.com/zach-snell/obsidian-go-mcp/releases) and add it to your PATH, or use:
...
Yes! Add multiple entries to your MCP config:
...
Or check the [Tools Overview](/obsidian-go-mcp/tools/overview).
...
obsidian-go-mcp doesn't have undo. Use:
...
obsidian-go-mcp can read and write Dataview inline fields (`key:: value`), but it doesn't execute Dataview queries. The AI can help you query your data using the search and frontmatter tools.
...
No. obsidian-go-mcp only handles text files (Markdown, JSON for canvas). Images and PDFs are not processed.

### Does it support wiki-style links?

Yes! obsidian-go-mcp fully supports `[[wikilinks]]` and `[[wikilinks|with aliases]]`. Tools like `backlinks`, `forward-links`, and `rename-note` understand and update wikilinks.

## Troubleshooting

### "Command not found: obsidian-go-mcp"
...
Open an issue on [GitHub](https://github.com/zach-snell/obsidian-go-mcp/issues) with:
- What you expected
- What happened
- Steps to reproduce
- obsidian-go-mcp version
...
Yes! See the [Development](/obsidian-go-mcp/advanced/development) guide for setup instructions and contribution guidelines.
